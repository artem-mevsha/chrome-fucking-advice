<template>
  <header class="ui ui_hoverable">
    <div class="ui__logo">
      <a class="ui__logo-text" v-if="isLogoEnabled" :href="baseUrl">Охуенный блять совет</a>

      <div class="ui__logo-tag" v-if="activeTag">
        <a class="ui__chapter" @click.prevent="resetTag">{{ activeTag.name }} </a>
      </div>
    </div>
    <div>
...
import FgaSubmenu from './Submenu';
...
.ui__menu-icon {
  ...
  margin-top: -1rem;
  z-index: 3;
  transition: margin-top 0.1s ease, opacity 0.2s ease, color 0.6s ease-in-out;
}
...
.ui__menu-icon:before {
  box-shadow: 0 -0.6rem 0 0 currentColor;
}
